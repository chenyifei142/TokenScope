...
前往 [GitHub Releases](https://github.com/chenyifei142/TokenSpider/releases/tag/v1.3.6) 下载 Windows 便携版：

- `TokenSpider-v1.3.6-windows-x64.exe`
...
```powershell
git clone https://github.com/chenyifei142/TokenSpider.git
cd TokenSpider
...
- 小米 MiMo Cookie 中自带的 `api-platform_ph` 会被自动提取并回填到同名输入框，避免在 Cookie 之外重复复制粘贴一次；请求头与 URL query 均以 Cookie 中的 `api-platform_ph` 为准，不会重复拼接。
- 粘贴小米 MiMo Cookie 时自动清除多余换行、制表与空白，保持 `name=value; name=value` 的标准格式。

### 验证

- 已补充 `api-platform_ph` 提取、Cookie 回退为独立凭据字段和 Cookie 空白规范化的回归测试。
- 已使用项目 `.venv` 执行 `python -m unittest discover -s tests -v`。
...
- 新增 GitHub Releases 在线更新能力，支持正式版 / 预发布版通道、启动后自动检查、手动检查、跳过当前版本和 24 小时缓存策略。
- 新增独立更新器 `TokenSpiderUpdater.exe`，主程序下载并校验完成后由更新器等待旧进程退出、备份、替换并重启新版。
- 新增 GitHub Actions 发布工作流与发布脚本，自动构建 `TokenSpider.exe`、`TokenSpiderUpdater.exe`、`SHA256SUMS.txt` 并上传到 GitHub Release。

### 优化

- 统一应用版本、主程序稳定文件名和 Release 附件命名，避免本地留下多个供用户手动选择的主程序 EXE。
- 设置页新增“软件更新”区域，展示当前版本、检查状态、更新通道和手动检查入口。
...
- 监控面板支持三层区域自由拖拽换位，并在重启后恢复用户保存的排序。
- 上层三张数据卡片支持层内拖拽换位，下层趋势图与使用统计支持层内互换，且禁止跨层拖动。
- 新增独立的 `panel-layout.json` 排序持久化文件，避免卡片排序写入影响凭据与普通配置备份流程。

### 优化

- 优化卡片拖拽交互，加入拖拽阈值、交换迟滞、占位框与平滑让位动画，减少跳位、抖动和生硬吸附。
- 层内拖拽时卡片尺寸规则与顺序解耦：上层始终保持一行三列等宽等高，下层趋势图始终保持宽卡片、使用统计始终保持窄卡片。
- 无效拖拽现在会平滑回弹，拖拽过程只复用现有卡片实例，不会频繁重建组件或提前写入排序配置。
